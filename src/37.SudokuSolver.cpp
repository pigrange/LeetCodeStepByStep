...
class Solution {
 ...
 private:
  bool doSolveSudoku(vector<vector<char>> &board, int pos) {
    while (pos < 81) {
      if (board[pos / 9][pos % 9] == '.') break;
      pos++;
    }
    if (pos >= 81) return true;
    int x = pos % 9, y = pos / 9;
    for (int i = 0; i < 9; i++) {
      if (used2[y][i] == 1 || used1[x][i] == 1 ||
          used3[x / 3 * 3 + y / 3][i] == 1)
        continue;
      ...
      used3[x / 3 * 3 + y / 3][i] = 1;
      board[y][x] = (char)(i + 49);
      if (doSolveSudoku(board, pos + 1) == true) return true;
      board[y][x] = '.';
      used1[x][i] = 0;
      used2[y][i] = 0;
      used3[x / 3 * 3 + y / 3][i] = 0;
...
//     // As we do it - constraints are propagated and even additional values
//     are
...
//     // however, if we have empty cells we need to use backtracking to fill
//     them if (!findValuesForEmptyCells()) return;  // sudoku is unsolvable
